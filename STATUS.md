...
- 2026-08-22 reconciliation: The two divergent lines (local v0.2.x and remote v0.1.3) were reconciled per owner decision (Option A: rebase local onto origin/master). Remote's CI/synthetic fixtures/DATA-CONTRACT kept as base; v0.2 feature set re-applied on top. Landed on `master` (tip `44f3515`, pushed, CI green); schema enforcement now unconditional per DATA-CONTRACT, 129 tests passing, `validate.py --strict` clean. Follow-up housekeeping: branch/worktree cleanup + CI actions bumped to node24.

## Open Items

- [x] Fold reconciliation into `master` and delete `reconcile-v021` (done 2026-08-22: master @ `44f3515`, branch deleted local+origin).
